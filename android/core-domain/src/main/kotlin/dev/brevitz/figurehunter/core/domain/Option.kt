...
sealed class Option<out A : Any> {
    ...
    inline fun <B : Any> map(mapper: (A) -> B): Option<B> = flatMap {
        Some(
            mapper(it)
        )
    }
    ...
    inline fun filter(predicate: (A) -> Boolean): Option<A> = flatMap { if (predicate(it)) Some(
        it
    ) else None
    }
}

fun <A : Any> A?.toOption(): Option<A> = if (this == null) Option.None else Option.Some(
    this
)
...
fun <A : Any, E : Any> Option<A>.toRemoteData(emptyValue: RemoteData<A, E> = RemoteData.Loading): RemoteData<A, E> =
    when (this) {
        ...
        is Option.Some -> RemoteData.succeed(
            value
        )
    }
